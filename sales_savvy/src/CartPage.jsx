...
const CartPage = () => {
  ...
  useEffect(() => {
    const fetchCartItems = async () => {
      try {
        ...
        console.log(data);
        setCartItems(
          data?.cart?.product.map((item) => {
            const pricePerUnit = item.price_per_unit || 799; // fallback price if missing
  ...
  const handleCheckout = async () => {
    try {
      ...
      const options = {
        key: "rzp_test_c6lBAbG697KEkS", // Replace with your Razorpay Key ID
...
// ✅ ✅ Mistake Fixed: Razorpay constructor is available now
